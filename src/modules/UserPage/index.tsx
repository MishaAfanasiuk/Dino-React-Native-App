...
const getChangeLink = (userId) => {
  return `https://dino-backend.herokuapp.com/user/${userId}/coins`
};

export const UserPage = inject('loginStore')(observer(({navigation, loginStore}) => {
  const user = loginStore.user;
  const [refreshing, setRefreshState] = useState(false);
  useEffect(() => {
    if (!loginStore.user)
        ...
        {(editMode ?
            ...
            >
              ...
              <View style={userStyles.buttonBlock}>
                ...
                <TouchableOpacity style={[userStyles.button, userStyles.green]}
                                  onPress={() => loginStore.editUser(state)}>
            ...
            <View>
              ...
              <TouchableOpacity style={userStyles.userCard} onPress={loginStore.displayingCard}>
                ...
                <QRCode value={getChangeLink(user?._id)} size={300}/>
              </TouchableOpacity>
            </View>
        )}
        <Dialog
